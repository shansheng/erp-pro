...
$(document).ready(function() {
	$('#matRequisitionTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/matoutrequisition/matOutRequisition/outData",
	  ...
	  if(navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)){//如果是移动端
		  ...
		  $('#matRequisitionTable').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
	           ...
	           $('#edit').prop('disabled', $('#matRequisitionTable').bootstrapTable('getSelections').length == 0);
  ...
  function deliverAll(){
		...
		jp.confirm('确认出库吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/matoutrequisition/matOutRequisition/deliverAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#matRequisitionTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
  ...
  function edit(id){//没有权限时，不显示确定按钮
  	 ...
  	 jp.openDialog('查看领料单', "${ctx}/matoutrequisition/matOutRequisition/form?id=" + id,'800px', '500px', $('#matRequisitionTable'));
  	  
  /*	jp.confirm('确认要出库吗？', function(){
		jp.loading();  	
		jp.get("${ctx}/matoutrequisition/matOutRequisition/outLocation?id=" +id, function(data){
			console.log(data);
     	  		if(data.success){
     	  			$('#matRequisitionTable').bootstrapTable('refresh');
     	  			jp.success(data.msg);
  ...
  function detailFormatter(index, row) {
	  ...
	  $.get("${ctx}/matoutrequisition/matOutRequisition/detail?id="+row.id, function(matRequisition){
