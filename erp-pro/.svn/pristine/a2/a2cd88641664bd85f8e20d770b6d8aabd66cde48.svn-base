...
<%@ attribute name="disabled" type="java.lang.String" required="false" description="是否限制选择，如果限制，设置为disabled"%>
	...
	<div class="input-group" style="width: 100%">
		<input id="${id}Code"  name="${labelName }" ${allowInput?'':'readonly="readonly"'}  type="text" value="${labelValue}" data-msg-required="${dataMsgRequired}"
...
$(document).ready(function(){
	$("#${id}Button, #${id}Code").click(function(){
		...
		top.layer.open({
		    ...
		    yes: function(index, layero){
		    	 ...
		    	 for(var i=0; i<items.length; i++){
		    		 var item = items[i];
		    		 var id = "#${id}".split("_")[0];
		    		 var num = parseInt("#${id}".split("_")[1])+i;
		    		 var type = "#${id}".split("_")[2];
		    		
		             if(typeof($(id+"_"+num+"_"+type+"Id").val()) != "undefined"){
		            	 $(id+"_"+num+"_"+type+"Id").val(item.id);
				    	 $(id+"_"+num+"_"+type+"Code").val(item${fn:substring(labelName, fns:lastIndexOf(labelName, '.'), fn:length(labelName))});
				    	 $(id+"_"+num+"_"+type+"Name").val(item.itemName);
				    	 $(id+"_"+num+"_"+"applyNumber").val("1");
		             }else{
		            	 addRow('#testTemporaryPlanListList', num, testTemporaryPlanListTpl); 
		            	 $(id+"_"+num+"_"+type+"Id").val(item.id);
				    	 $(id+"_"+num+"_"+type+"Code").val(item${fn:substring(labelName, fns:lastIndexOf(labelName, '.'), fn:length(labelName))});
				    	 $(id+"_"+num+"_"+type+"Name").val(item.itemName);
				    	 $(id+"_"+num+"_"+"applyNumber").val("1");
		             ...
		             //删除中间记录情况
		             if(testTemporaryPlanListRowIdx < num){
		            	 testTemporaryPlanListRowIdx = num; 
		             }
