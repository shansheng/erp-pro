...
<head>
	...
	<%@include file="mdsProcurementPlanFinishList.js" %>
</head>
<body>
	...
	<div id="collapseTwo" class="accordion-body collapse">
		<div class="accordion-inner">
			<form:form id="searchForm" modelAttribute="mdsProcurementPlanFinish" class="form form-horizontal well clearfix">
			 ...
			 <div class="col-xs-12 col-sm-6 col-md-4">
				<label class="label-item single-overflow pull-left" title="来源类型：">来源类型：</label>
				<select id="sourceType" name="sourceType" class="form-control m-b">
					<option value="" selected="selected">---请选择---</option>
					<option value="项目录入">项目录入</option>
					<option value="临时录入">临时订单</option>
					<option value="手动录入">手动录入</option>
				</select>
			</div>
			 <div class="col-xs-12 col-sm-6 col-md-4">
				<label class="label-item single-overflow pull-left" title="采购状态：">采购状态：</label>
				<form:select path="finishState" class="form-control m-b">
					<form:option value="" label="---请选择---" />
					<form:options
						items="${fns:getDictList('procurement_finish_state')}"
						itemLabel="label" itemValue="value" htmlEscape="false" />
					</form:select>
			</div>
			<div class="col-xs-12 col-sm-6 col-md-4">
				<span>产品所在合同</span>
					<form:select path="fromId" class="form-control m-b">
						<form:option value="" label="请选择" />
						<form:options items="${fns:getSqlList('SELECT DISTINCT mesewo.from_id AS VALUE, mesewo.particulars AS label FROM mds_procurement_plan_v mesewo WHERE mesewo.FROM_ID IS NOT NULL')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
					</form:select>
			</div>
			<div class="col-xs-12 col-sm-6 col-md-4">
		 ...
		 <div class="col-xs-12 col-sm-6 col-md-4">
			 ...
			 <div class="col-xs-12 col-sm-6 col-md-4">
				<label class="label-item single-overflow pull-left" title="采购人：">采购人：</label>
				<form:input path="buyer.name" htmlEscape="false" maxlength="100"  class=" form-control"/>
			</div>
		 <div class="col-xs-12 col-sm-6 col-md-4">
