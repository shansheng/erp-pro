...
$(document).ready(function() {
	...
	$('#mdsProcurementPlanTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/mds/mdspcmplanprocurement/data",
	  ...
	  $('#mdsProcurementPlanTable').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
		  	...
		  	$('#edit').prop('disabled', ! ($('#mdsProcurementPlanTable').bootstrapTable('getSelections').length>0));
  ...
  function edit(){//没有权限时，不显示确定按钮
	  ...
	  if(date== -1){
		  jp.error("审批数量或需求时间不能为空!");
	  }else if(date == 0){
		  jp.error("审批数量不能为负数或0!");
	  }else{
		  jp.confirm('确认要保存采购计划吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/mds/mdspcmplanprocurement/save?date="+ date, function(data){
		         	if(data.success){
		         	  	$('#mdsProcurementPlanTable').bootstrapTable('refresh');
		         	  	jp.success(data.msg);
		         	  }else{
  ...
  function collectAll(){
   ...
   //驳回
   function reject(){
		jp.confirm('确认要驳回采购计划吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/mds/mdspcmplanprocurement/reject?ids=" + getIdSelections(), function(data){
		         	if(data.success){
		         	  	$('#mdsProcurementPlanTable').bootstrapTable('refresh');
		         	  	jp.success(data.msg);
		         	  }else{
   ...
   function addIndent(){
	   ...
	   if(date== -1){
		...
		jp.confirm('确认要下达采购订单吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/mds/mdspcmplanprocurement/addIndent?date=" + date, function(data){
		         	if(data.success){
		         	  	$('#mdsProcurementPlanTable').bootstrapTable('refresh');
		         	  	jp.success(data.msg);
		         	  }else{
