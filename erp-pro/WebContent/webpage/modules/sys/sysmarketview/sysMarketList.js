...
$(document).ready(function() {
	$('#sysMarketTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/sysmarketview/sysMarket/data",
                   ...
                   if($el.data("item") == "edit"){
                   	edit(row.id);
                        ...
                        jp.confirm('确认要删除该销售记录吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/sysmarketview/sysMarket/delete?id="+row.id, function(data){
                   	  		if(data.success){
	  ...
	  $('#sysMarketTable').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
            ...
            $('#dispatch').prop('disabled', $('#sysMarketTable').bootstrapTable('getSelections').length!=1);
            $('#kp').prop('disabled', $('#sysMarketTable').bootstrapTable('getSelections').length!=1 || $('#sysMarketTable').bootstrapTable('getSelections')[0].isKp!='0');
        });
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  window.location='${ctx}/sysmarketview/sysMarket/import/template';
				  },
  ...
  function deleteAll(){
   ...
   function add(){
	  jp.openDialog('新增销售', "${ctx}/sysmarketview/sysMarket/form",'800px', '500px', $('#sysMarketTable'));
  }
  function edit(id){//没有权限时，不显示确定按钮
  	  if(id == undefined){
			id = getIdSelections();
		}
	   <shiro:hasPermission name="sysmarketview:sysMarket:edit">
	  jp.openDialog('编辑销售', "${ctx}/sysmarketview/sysMarket/form?id=" + id,'800px', '500px', $('#sysMarketTable'));
	   </shiro:hasPermission>
	  <shiro:lacksPermission name="sysmarketview:sysMarket:edit">
	  jp.openDialogView('查看销售', "${ctx}/sysmarketview/sysMarket/form?id=" + id,'800px', '500px', $('#sysMarketTable'));
  ...
  function view(id){
	  ...
	  jp.openDialogView('查看销售', "${ctx}/sysmarketview/sysMarket/view?id=" + id,'800px', '500px', $('#sysMarketTable'));
  }
  ...
  function dispatch(id){
		jp.confirm('确认要下达吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/sysmarketview/sysMarket/dispatch?id=" + getIdSelections(), function(data){
   	  		if(data.success){
   	  			$('#sysMarketTable').bootstrapTable('refresh');
   	  			jp.success(data.msg);
   	  		}else{
