								...
								.validate(
										{
											submitHandler : function(form) {
												jp
														.post(
																"${ctx}/SysMarketShip/SysMarketShip/save",
																$('#inputForm')
																		.serialize(),
																function(data) {
																	if (data.success) {
...
<body class="bg-white">
	<form:form id="inputForm" modelAttribute="sysMarketShip"
			...
			<tbody>
				...
				<tr>
					...
					<td class="width-35"><form:input path="contractNo" disabled="true"
							...
							<td class="width-15 active"><label class="pull-right"><font
							color="red">*</font>是否发货：</label></td>
					<td class="width-35" ><form:select path="shipments" disabled="true"
				...
				<tr>
				   <td class="width-15 active"><label class="pull-right">发货记录:</label></td>
				   <td class="width-35" colspan="3">
				      <textarea name="shipmentsRecord" style="resize:none;margin: 0px; height: 50px; width: 635px;">${sysMarket.shipmentsRecord}</textarea>
				   </td>
				</tr>
			...
			<table class="table table-striped table-bordered table-condensed">
				<thead>
					<tr>
						...
						<th style="width:10%;">累计发货数</th>
						<th style="width:10%;"><font color="red">*</font>发货数</th>
					</tr>
				</thead>
...
//<!--
				<tr id="sysMarketShipListList{{idx}}">
					<td class="hide">
						<input id="sysMarketShipListList{{idx}}_id" name="sysMarketShipListList[{{idx}}].id" type="hidden" value="{{row.id}}"/>
						<input id="sysMarketShipListList{{idx}}_delFlag" name="sysMarketShipListList[{{idx}}].delFlag" type="hidden" value="0"/>
					</td>
					...
					<td >
<input id="sysMarketShipListList{{idx}}_mdsItem" name="mdsItem.name" type="text" value="{{row.itemName}}"    class="form-control " readonly="true"/>


					<td>
						<input id="sysMarketShipListList{{idx}}_mdsItemspecs" name="mdsItem.specs" type="text" value="{{row.itemSpecs}}"    class="form-control " readonly="true"/>
					</td>
					<td>
						<input id="sysMarketShipListList{{idx}}_mdsItemmodelNum" name="mdsItem.modelNum" type="text" value="{{row.modelNum}}"    class="form-control " readonly="true"/>
					</td>
					<td>
						<input id="sysMarketShipListList{{idx}}_num" name="sysMarketShipListList[{{idx}}].num" type="text" value="{{row.num}}"    class="form-control required number" readonly="true"/>
					</td>
					<td>
						<input id="sysMarketShipListList{{idx}}_deliverNums" name="sysMarketShipListList[{{idx}}].deliverNums" type="text" value="{{row.deliverNums}}" readonly="true"   class="form-control  number" />
					</td>
					<td>
						<input id="sysMarketShipListList{{idx}}_deliverNum" name="sysMarketShipListList[{{idx}}].deliverNum" type="text" value="{{row.deliverNum}}"    class="form-control required number" />
					</td>
				</tr>//-->
			</script>
			<script type="text/javascript">
				...
				$(document).ready(function() {
					var data = ${fns:toJson(sysMarket.sysMarketShipListList)};
