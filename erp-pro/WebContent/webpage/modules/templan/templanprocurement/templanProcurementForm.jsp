				...
				alert("请选择一条记录!");
				 ...
				 var check_val = [];
				   ...
				   check.each(function(){
					   ...
					   var shipmentsStatus = row.find("[name='tempShipmentsStatus']").val();
					   var temporaryPlanId = row.find("[name='temporaryPlanId']").val();
					   if(shipmentsStatus == "已发货"){
						   flag = true;
					   }
					   
					   check_val.push( {id:id,remarks:remarks,temporaryPlanId:temporaryPlanId});
				   }) 
			   
			if(flag){
			      alert("该物料已经发货，不能进行重复发货操作！");
			} else {
				$.ajax({
...
<body class="bg-white">
		...
		<div class="tabs-container">
			...
			<a class="btn btn-white btn-sm" onclick="deliver()" title="发货"><i class="fa fa-plus"></i>发货</a>
			</shiro:hasPermission>
			<table class="table table-striped table-bordered table-condensed" id="tableId">
				<thead>
					<tr>
					    <th>
					    <!-- <input id="allbox" onclick="allCheck()" type="checkbox"/> -->
					    </th>
						<th class="hide"></th>
			...
			<script type="text/template" id="templanProcurementListTpl">//<!--
				<tr id="templanProcurementListList{{idx}}">
					...
					<td class="hide">
						<input id="templanProcurementListList{{idx}}_id" name="tempId" type="hidden" value="{{row.id}}"/>
						<input id="templanProcurementListList{{idx}}_delFlag" name="templanProcurementListList[{{idx}}].delFlag" type="hidden" value="0"/>
						<input id="templanProcurementListList{{idx}}_temporaryPlanId" name="temporaryPlanId" type="hidden" value="{{row.temporaryPlanId}}"/>
					</td>
					...
					<td>
                       <input id="templanProcurementListList{{idx}}_shipmentsStatus" name="tempShipmentsStatus" type="text" value="{{row.shipmentsStatus}}" readonly="true" class="form-control"/>
                     </td>
					
					<td>
						<textarea id="templanProcurementListList{{idx}}_remarks" name="tempRemarks" rows="2"      class="form-control">{{row.remarks}}</textarea>
					</td>
				</tr>//-->
			</script>
			<script type="text/javascript">
