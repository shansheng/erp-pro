...
$(document).ready(function() {
	$('#workOrderSonTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/WorkOrderRetur/WorkOrderRetur/data",
	  ...
	  $('#workOrderSonTable').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
            ...
            $('#edit').prop('disabled', $('#workOrderSonTable').bootstrapTable('getSelections').length!=1);
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该工作计划记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/workorderson/workOrderSon/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#workOrderSonTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
  ...
  function add(){
	   ...
	   window.location = "${ctx}/WorkOrderReturDetail/WorkOrderReturDetail/list?workOrderSonId=" +id+"&workOrderId="+$("#workOrderId").val();
}
...
function view(){
	 ...
	 window.location = "${ctx}/WorkOrderReturDetail/WorkOrderReturDetail/lists?workOrderSonId=" +id+"&workOrderId="+$("#workOrderId").val();
