		...
		$(document).ready(function() {
			validateForm = $("#inputForm").validate({
				submitHandler: function(form){
					jp.post("${ctx}/matdeputy/matDeputy/save",$('#inputForm').serialize(),function(data){
						if(data.success){
...
<body class="bg-white">
		<form:form id="inputForm" modelAttribute="matRequisition" action="${ctx}/matdeputy/matDeputy/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
				...
				<tr>
						...
						<th colspan="2">
							<form:select path="statying" class="form-control m-b required">
								...
								<form:options items="${fns:getDictList('mat_deputy_state')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
							</form:select>
						</th>
						<th colspan="4">
						</th>
			...
			<script type="text/template" id="matRequisitionListTpl">//<!--
				<tr id="matRequisitionListList{{idx}}">
					...
					<sys:proItemSelectForForm url="${ctx}/mdsitem/mdsItem/data?itemType=M"
										id="matRequisitionListList{{idx}}_item" name="matRequisitionListList[{{idx}}].mdsItemId" value="{{row.mdsItemId}}"
										labelName="mdsItem.itemCode" labelValue="{{row.itemCode}}"
