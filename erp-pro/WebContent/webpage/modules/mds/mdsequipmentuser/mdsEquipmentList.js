...
$(document).ready(function() {
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	edit(row.id);
                        ...
                        jp.confirm('确认要删除该设备人员记录吗？', function(){
                       	jp.loading();
			...
			,{
                ...
                events: {
    		        'click .assign': function (e, value, row, index) {
    					...
    					$("#mdsEquipmentId").val(row.id);
    					$('#userTable').bootstrapTable("refresh",{query:{mdsEquipmentId:row.id}})
    		        }
    		    },
                formatter:  function operateFormatter(value, row, index) {
    		        return [
						<shiro:hasPermission name="mdsequipmentuser:mdsEquipment:add"> 
  ...
  function deleteAll(){
   ...
   function add(){
	  jp.openDialog('新增设备人员', "${ctx}/mdsequipment/mdsEquipment/form",'800px', '500px', $('#mdsEquipmentTable'));
  }
  function edit(id){//没有权限时，不显示确定按钮
	  ...
	  jp.openDialog('编辑设备人员', "${ctx}/mdsequipment/mdsEquipment/form?id=" + id,'800px', '500px', $('#mdsEquipmentTable'));
	   </shiro:hasPermission>
	  <shiro:lacksPermission name="mdsequipment:mdsEquipment:edit">
	  jp.openDialogView('查看设备人员', "${ctx}/mdsequipment/mdsEquipment/form?id=" + id,'800px', '500px', $('#mdsEquipmentTable'));
