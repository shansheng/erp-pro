...
$(document).ready(function() {
	$('#mdsReplaceItemTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/mds/mdsReplaceItem/approveList",
	  ...
	  $('#mdsReplaceItemTable').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
            ...
            $('#approve').prop('disabled',$('#mdsReplaceItemTable').bootstrapTable('getSelections').length!=1);
            $('#reject').prop('disabled',$('#mdsReplaceItemTable').bootstrapTable('getSelections').length!=1);
        });
  ...
  function deleteAll(){
    ...
    */
   function approve(){
	    var row = $('#mdsReplaceItemTable').bootstrapTable('getSelections');
	    console.log("itemId:"+row[0].itemId);
	    jp.post("${ctx}/mds/mdsReplaceItem/agree",
	    		{
	            id:row[0].id,
	    	    itemId:row[0].itemId,
	    	    itemName:row[0].itemName,
	    	    afterReplacePuraseNumber:row[0].afterReplacePuraseNumber,
	    	    mdsProcurementPlanId:row[0].mdsProcurementPlanId
	    	     },function(data){
	    			if(data.success){
	    				$('#mdsReplaceItemTable').bootstrapTable('refresh');
	    				jp.success(data.msg);
   ...
   function reject(id){
	   if(id==undefined){
		   ...
		   jp.get("${ctx}/mds/mdsReplaceItem/reject?id="+id,function(data){
			   if(data.success){
				   $('#mdsReplaceItemTable').bootstrapTable('refresh');
				   jp.success(data.msg)
			   }else{
